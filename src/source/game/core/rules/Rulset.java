...
public class Rulset {
    ...
    private ArrayList<Card> applyCol2Col(Card first, ArrayList<Card> pCards) {
        if (!col2col) { return pCards; }
        if (gametype.isTrumpf(first) && excludeTrunpf) { return pCards; }
        ...
        if (newCards.size()==0) { return pCards; }

        return newCards;
    }

    private ArrayList<Card> applyTrumpf2Trumpf(Card first, ArrayList<Card> pCards) {
        if (!trumpf2trumpf || !gametype.isTrumpf(first)) { return pCards; }
        ArrayList<Card> newCards = new ArrayList<>();
